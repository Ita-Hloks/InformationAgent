...
这是 Information Agent 的独立前端目录，默认通过 Vite 代理连接本地 API；订阅、文章、已读和收藏状态由 FastAPI 服务写入 SQLite，研究工作区可创建采集运行并基于已入库证据运行 Agent。当前没有登录系统，不提供多用户隔离或跨设备同步。

研究工作区会恢复所选运行最近一次持久化的 Agent 报告；没有历史报告时保持空结果，不会自动触发新的 Agent 运行。
