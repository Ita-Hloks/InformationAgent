...
主题级“任意 RSS/Atom -> 采集入库 -> 证据集 -> Agent”业务已经废除。前端没有独立研究页面、全局研究运行列表或主题采集入口；舆情分析仍是独立保留业务。
...
已删除 `ingest`、`plan-run`、`list-runs` 和 `agent-run`，不保留兼容命令。
...
旧 `/api/research/*` 主题级路由已删除并返回 `404`。
